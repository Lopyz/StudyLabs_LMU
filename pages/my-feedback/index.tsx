...
import { useGetUserFilesClerkID } from '@/hooks/useGetUserFilesClerkID';
...
export default function Settings() {
  ...
  const handleNewProject = () => { router.push('/my-tasks'); };
  // Diese Funktion wird aufgerufen, nachdem die Dateien heruntergeladen wurden
  const resetSelectedFilesAfterDownload = () => {
    setSelectedFiles([]); // Setzt den Zustand zurück, sodass keine Dateien ausgewählt sind
  };

  const { data, isLoading, isError, revalidateData } = useGetUserFilesClerkID();
      ...
      <Card w="100%" mb="20px">
        <Flex align="center" direction={{ base: 'column', md: 'row' }}>
          <Text fontSize="lg" fontWeight={'700'} mr={"10px"}>
            {language === 'DE' ? `Mein KI-Feedback  (${sortedData ? sortedData.length : 0})` : `My AI-Feedback (${sortedData ? sortedData.length : 0})`}
          </Text>
          <Tooltip label={language === 'DE' ? 'Zeigt eine Liste aller Anfragen, die du bearbeitet hast. Die Zahl in Klammern gibt an, wie viele Anfragen aktuell vorliegen.' : 'Shows a list of all requests that you have processed. The number in brackets indicates how many requests are currently available.'} placement="top">
            <span>
